...
import (
	...
	"github.com/llm-d/llm-d-router/pkg/epp/framework/plugins/scheduling/filter/sessioncontrol"
)
...
func TestFilter(t *testing.T) {
	...
	filter := sessioncontrol.NewSessionControl("test")
	...
	tests := []struct {
		...
		{
			name:      "no binding passes all candidates",
			request:   &scheduling.InferenceRequest{},
			endpoints: []scheduling.Endpoint{epA, epB},
			want:      []scheduling.Endpoint{epA, epB},
		},
		{
			name:      "binding narrows to the bound endpoint",
			request:   requestBoundTo(podA),
			endpoints: []scheduling.Endpoint{epA, epB},
			want:      []scheduling.Endpoint{epA},
		},
		{
			name:      "bound endpoint absent falls through to all candidates",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := filter.Filter(context.Background(), tc.request, tc.endpoints)
...
func TestConsumesSessionBinding(t *testing.T) {
	t.Parallel()
	filter := sessioncontrol.NewSessionControl("test")
	_, ok := filter.Consumes().Required[attrsession.SessionBindingDataKey]
